...
(@)  **Sensitivity analysis**    
    ...
    1. Construct a dendrogram using the **single linkage** approach.  
...
```
    2. Construct a dendrogram using the **centroid linkage** approach. 
...
```
    ...
    3. Construct a dendrogram using the **average linkage** approach.
...
```
    ...
    d. What type of linkage is most reasonable for your analysis? Justify your choice based on the data context and your scientific goals.    
...
Now let's try the K-means approach.  The first step is to **pick K**.  So how do we choose?    
    1. Run and store a K-means algorithm using K = 2.    
...
```
    ...
    2. One strategy for evaluating and comparing K is to calculate the total squared distance of each case from its assigned centroid (the `total within- cluster sum of squares`).      
...
```
    ...
    3. Next, we calculate the total sum of squared distances for each K in $\{1, 2, ..., n\}$ where we pick a reasonable $n$. 
```{r}
tune_data <- data.frame(K = 1:2, SS)
tune_data
...
(@) **A final K-means model**    
    We run the K-means algorithm using the K we identified earlier.  Store the cluster labels in your data set.     
...
```
    ...
    To get a sense for how some of the features might have played into the clustering, calculate the mean of each feature within each cluster.    
...
(@) **Final reflection**    
        ...
        My final cluster is 5 clusters. After five the sum of squared distances decreases relatively little with each new cluster split added. There are few clusters that could be characterized as being caramel, chocolate, fruity, peanutyalmondy, and nougat.
...
```
    ...
    e. Based on the loadings plot, these seem to be split up very similar to how we saw it in the prevous plot with the four quardrants having similar themes as the last graph. 
